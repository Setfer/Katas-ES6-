...
const favoritesSoundsArr = []
for (user of users) {
  for (Sound in user.favoritesSounds) {
    favoritesSoundsArr.push(Sound)
  }
}
console.log(favoritesSoundsArr)

  const soundgrupo = {}

  for (sound of favoritesSoundsArr) {
    if (!soundgrupo[sound]) {
      soundgrupo[sound] = []
    }
    soundgrupo[sound].push(sound)
  }
console.log(soundgrupo)

const sumaSound={}
for (sound in soundgrupo){
  sumaSound[sound] = soundgrupo[sound].length;
}
console.log(sumaSound)